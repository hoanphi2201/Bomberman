 ...
 * @author phixuanhoan
 */
public class test extends AI{
    private Enemy _e;
    private Board _board;

    public test(Bomber bomber, Enemy e, Board b) {
        _e = e;
        _board = b;
    }
    ...
    private int directionHasBomb(int distance) {
        ...
        for (int i = 0; i < distance; i++)
            if (_board.getEntityAt(x, y - 1 - i) != null) return 0;
        for (int i = 0; i < distance; i++)
            if (_board.getEntityAt(x + 1 + i, y) != null) return 1;
        for (int i = 0; i < distance; i++)
            if (_board.getEntityAt(x, y + 1 + i) != null) return 2;
        for (int i = 0; i < distance; i++)
            if (_board.getEntityAt(x - 1 - i, y) != null) return 3;
